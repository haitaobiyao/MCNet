...
def generate_random_crop_pos(ori_size, crop_size):
    ...
    if h > crop_h:
        pos_h = random.randint(0, h - crop_h + 1) #用于生成一个指定范围内的整数
...
def contrast_and_Brightness(img):
    alpha = random.choice([1, 1.2, 1.4, 1.6, 1.8])
    beta = random.choice([1, 2, 4, 8])
...
def add_saliency(img, gt):

    chance = random.choice([0, 1])
    if chance == 1:
        img = convert_image_to_saliency(img, gt)

    return img

def convert_image_to_saliency(img, label):
    thr_img = doDualThreshold(img)
    temp_imgR = img[:, :, -1]
    temp_imgG = img[:, :, 1]
    temp_imgB = img[:, :, 0]
    # print(temp_imgB.shape[0])
    for i in range(temp_imgB.shape[0]):
        for j in range(temp_imgB.shape[1]):
            if j < 25 or j + 36 > temp_imgB.shape[1]:
                continue
            if label[i, j] != 24 and label[i, j] != 25 and label[i, j] != 21:
                continue
            if thr_img[i, j] == 255:
                if temp_imgB[i, j] > 210:
                    temp_imgB[i, j] = 255
                else:
                    temp_imgB[i, j] = temp_imgB[i, j] + 40
            if thr_img[i, j] == 255:
                if temp_imgG[i, j] > 210:
                    temp_imgG[i, j] = 255
                else:
                    temp_imgG[i, j] = temp_imgG[i, j] + 40
            if thr_img[i, j] == 255:
                if temp_imgR[i, j] > 210:
                    temp_imgR[i, j] = 255
                else:
                    temp_imgR[i, j] = temp_imgR[i, j] + 40

    saliency_img = np.concatenate((temp_imgB.reshape((576, 720, 1)), temp_imgG.reshape((576, 720, 1))), axis=2)
    saliency_img = np.concatenate((saliency_img, temp_imgR.reshape((576, 720, 1))), axis=2)
    return saliency_img

def doDualThreshold(im):
    OMEGA = 13
    ALPHA = 1
    KAY = 1.06
    EPSILON = 4
    ...
    if im.shape[2] != 1:  # convert color image to gray image
