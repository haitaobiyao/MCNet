...
class ShareSeqConv(nn.Module):  # get local information before dilation conv
    def __init__(self, kernel_size):
        super(ShareSeqConv, self).__init__()
        assert kernel_size % 2 == 1, "kernel size should odd"
    ...
    def forward(self, x):
        input_chan = x.size(1)
        expand_weight = self.weight.expand(input_chan, 1, self.kernel_size, self.kernel_size).contiguous()
        return F.conv2d(x, expand_weight,
                        None, 1, self.padding, 1, input_chan)


class GatedConv2dWithActivation(nn.Module):  # Gated Conv

    def __init__(self, inc, outc, kernel_size, stride=1,
                 pad=0, dilation=1, groups=1, bias=False,
                 batch_norm=True, activation=nn.LeakyReLU(0.2, inplace=True),
                 norm_layer=nn.BatchNorm2d, bn_eps=1e-5):
        super(GatedConv2dWithActivation, self).__init__()
        self.batch_norm = batch_norm
        self.activation = activation
        self.conv2d = nn.Conv2d(inc, outc, kernel_size, stride, pad, dilation, groups, bias)
        self.mask_conv2d = nn.Conv2d(inc, outc, kernel_size, stride, pad, dilation, groups, bias)
