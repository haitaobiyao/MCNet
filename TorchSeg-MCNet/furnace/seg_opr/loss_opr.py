...
class FocalLoss(nn.Module):
    ...
    def forward(self, pred, target):
        n, c, h, w = pred.size()
        criterion = torch.nn.CrossEntropyLoss(ignore_index=self.ignore_label, reduction='none')
...
class EdgeLoss(nn.Module):
    def __init__(self, ignore_label, reduction='mean'):
    ...
    def forward(self, pred, target):
        ...
        return F.binary_cross_entropy_with_logits(pred, target, weights, reduction=self.reduction)
